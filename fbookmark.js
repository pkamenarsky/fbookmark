...
group.commands.add(["fbmark", "fbm"], "Save current page as bookmark in folder",
		...
		function(item) {
			...
			if (options.get("fbwebloc")) {
				bookmark = io.File(options.get("fbpath").stringValue).child("" + item).child(document.title + ".webloc");
				bookmark.write(
				...
				bookmark = io.File(options.get("fbpath").stringValue).child("" + item).child(document.title + ".url");
				bookmark.write(
					"[InternetShortcut]\r\n" +
